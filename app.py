"""
DD Predictor — Flask Backend API v3
=====================================
Uses CSV data for both training AND fixtures.
No external API dependency — works on any server.
"""
...
def get_df():
    ...
    if _df_cache is None:
        ...
        # Assign gameweek numbers within each season (10 games per GW)
...
def reload_data():
    """Force reload of data and model — call after updating CSV"""
...
def predict_match(home_team, away_team, model, hfm=1.0, afm=1.0):
    atk, dfc, ha = model["attack"], model["defence"], model["home_advantage"]
    if home_team not in atk or away_team not in atk:
        return None
    lh = np.exp(ha + atk[home_team] + dfc[away_team])
    la = np.exp(atk[away_team] + dfc[home_team])
    lh = max(lh * (1 + FORM_WEIGHT * (hfm - 1)), 0.1)
    la = max(la * (1 + FORM_WEIGHT * (afm - 1)), 0.1)
    ...
    scores = [{"score": f"{i//(MAX_GOALS+1)}-{i%(MAX_GOALS+1)}",
               "probability": round(float(pm[i//(MAX_GOALS+1)][i%(MAX_GOALS+1)])*100, 1)}
              for i in top5]
...
def build_predictions_from_rows(rows, gameweek, df, model, season):
    """Build prediction output from dataframe rows"""
    fixtures = []
    for _, m in rows.iterrows():
        ht = m["HomeTeam"]
        at = m["AwayTeam"]
        hf = get_form(df[df["Date"] < m["Date"]], ht)
        af = get_form(df[df["Date"] < m["Date"]], at)
        pred  = predict_match(ht, at, model, hf["multiplier"], af["multiplier"])
        cards = get_cards(df[df["Date"] < m["Date"]], ht, at)
        h2h   = get_h2h(df[df["Date"] < m["Date"]], ht, at)
        ...
        if pred:
            fixtures.append({
                "fixture_id":   f"{ht}-{at}-{m['Date'].date()}",
                "match_date":   m["Date"].strftime("%Y-%m-%d"),
                "match_time":   "",
...
def seasons():
    """Return available seasons and gameweek counts"""
    df = get_df()
    result = []
...
def predictions(gameweek):
    """
    Load predictions for a gameweek from CSV data.
    Uses the most recent season by default.
    Pass ?season=2025-26 to specify a season.
    """
    try:
        df     = get_df()
        model  = get_model()
        ...
        if len(rows) == 0:
            # Try to find closest available GW
            available = sorted(df[df["Season"]==season]["GW"].unique())
            return jsonify({
                "error": f"Gameweek {gameweek} not found in {season}. Available: {available}"
            }), 404

        return build_predictions_from_rows(rows, gameweek, df, model, season)
...
def reload():
    """Reload data and refit model — call after updating CSV"""
    try:
        model = reload_data()
        df    = get_df()
        return jsonify({
...
def team_stats(team_name):
    try:
        ...
        h["Result"]=h["FTR"].map({"H":"W","D":"D","A":"L"}); h["Yellows"]=h["HY"]
        ...
        a["Result"]=a["FTR"].map({"A":"W","D":"D","H":"L"}); a["Yellows"]=a["AY"]
